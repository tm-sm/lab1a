...
void vWatchdogTask(void *args) {
    ...
    while (true) {
        int e = xTaskNotifyWait(0, 0, 0, pdMS_TO_TICKS(1000));
        if (e) {
            overloaded = true;
...
void vButtonTask(void *args) {
    ...
    for (;;) {
        ...
        value_brake_pedal = BSP_GetInput(SW_5);
        value_cruise_control = BSP_GetInput(SW_6);
        value_gas_pedal = BSP_GetInput(SW_7);
...
void vControlTask(void *args) {
    ...
    for (;;) {
        ...
        cruise_control = cruise_control_button;
...
void vVehicleTask(void *args) {
    ...
    for (;;) {
        ...
        accel += alpha * (BSP_GetAxisAcceleration(Z_AXIS) - accel);
        if (accel < g && accel > 0) {
            slope = acosf(accel / g) / (3.1415 / 2.0) * 32;
        }
...
void vDisplayTask(void *args) {
    ...
    for (;;) {
        ...
        BSP_7SegDispInt(slope * 100 + velocity);

        uint8_t buf[3] = {0, 0, 0};
        for (uint8_t i = 0; i < position / 100; i++) {
            buf[i / 8] |= 1 << (i % 8);
        }
...
int main()
{
    ...
    xTaskCreate(vWatchdogTask, "Watchdog Task", 512, NULL, 7, &xWatchdog_handle);
    xTaskCreate(vOverloadTask, "Overload Task", 512, NULL, 1, &xOverload_handle);
